...
public class CadastroPFController {
	...
	public ResponseEntity<Response<CadastroPFDto>> cadastrar(
		...
		log.info("Erro validando dados de cadastro PF: {}", cadastroPFDto.toString());
	...
	private Funcionario converterDtoParaFuncionario(
			@Valid CadastroPFDto cadastroPFDto, BindingResult result
	) throws NoSuchAlgorithmException {
	...
	/*
	 * Verifica se a empresa está candastrada e se o funcionário não existe na base de dados.
	 * 
	 * @param cadastroPFDto
	 * @param result
	*/
	private void validarDadosExistentes(@Valid CadastroPFDto cadastroPFDto, BindingResult result) {
		...
		if (!empresa.isPresent()) result.addError(new ObjectError("empresa", "Empresa não cadastrada."));
